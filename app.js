...
function fetchUser(id) {
  return fetch(`http://localhost:3000/api/v1/users/${id}`)
    ...
    .then(userData => populateStore(userData.attributes.climbs))
    .then(() => fillTabulator(store.climbs))
    .catch(error => console.error(error.message))
}

function populateStore(climbs) {
  const climbsWithButtons = climbs.forEach(function(climb) {
    return addButtons(climb)
  })
  console.log(climbsWithButtons)
}

function addButtons(climb) {
  const
}
